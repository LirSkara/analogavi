...
@endsection
@section('content')
    <div class="container py-lg-5" id="edit_image_tour">
        <div class="d-flex">
        ...
        <div class="d-flex flex-wrap gap-2 mt-3">
            <div v-for="(item, id) in images" :key="item" class="cards shadow-sm bg-white border rounded-3 mt-2">
                <img class="img-fluid rounded-top img-cover w-100" :src="'/storage/tours_image/{{$item->user}}/' + item" alt="...">
                <div class="d-flex">
    ...
    <script>
        const EditImageTour = {
            ...
            methods: {
                ...
                delete_images(id) {
                    axios({
                        method: 'post',
                        url: `/delete_image_tour/{{$item->id}}/${id}`,
                        responseType: 'json',
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
                ...
                edit_images(id) {
                    ...
                    if(document.getElementById(`edit_file${id}`).files.length != 0) {
                        ...
                        axios({
                            method: 'post',
                            url: `/edit_image_tour/{{$item->id}}/${id}`,
                            responseType: 'json',
        ...
        Vue.createApp(EditImageTour).mount('#edit_image_tour')
    </script>
@endsection