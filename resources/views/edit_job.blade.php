@extends('layout')
@section('title')
    Khalif - Редактирование работы
@endsection
@section('content')
    
    <div class="container" id="edit_job">
        <div v-if="type_job != ''">
            ...
            <div v-if="job == 'Резюме'" class="d-flex flex-column">
                ...
                <button class="btn btn-primary w-700 mt-3" v-on:click="add_job">Редактировать</button>
            </div>
    ...
    <script>
        const EditJob = {
            data() {
                return {
                    ...
                    year: '',
                    desired_position: '{{$job->desired_position}}',
                    work_schedule: '{{$job->work_schedule}}',
                    work_experience: '{{$job->work_experience}}',
                    education: '{{$job->education}}',
                    gender: '{{$job->gender}}',
                    day_birth: '{{$job->day_birth}}',
                    month_birth: '{{$job->month_birth}}',
                    year_birth: '{{$job->year_birth}}',
                    read_trips: '{{$job->read_trips}}',
                    move: '{{$job->move}}',
                    citizenship: '{{$job->citizenship}}',
                    company_name: '{{$job->company_name}}',
                    post: '{{$job->post}}',
                    month_getting_started: '{{$job->month_getting_started}}',
                    year_getting_started: '{{$job->year_getting_started}}',
                    month_end_work: '{{$job->month_end_work}}',
                    year_end_work: '{{$job->year_end_work}}',
                    until_now: '{{$job->until_now}}',
                    responsibilities: '{{$job->responsibilities}}',
                    name_institution: '{{$job->name_institution}}',
                    specialization: '{{$job->specialization}}',
                    year_graduation: '{{$job->year_graduation}}',
                    knowledge_languages: '{{$job->knowledge_languages}}',
                    ...
                    description_error: '',
                    adres_error: '',
                    from_price: '{{$job->from_price}}',
                    before_price: '{{$job->before_price}}',
                    when_price: '{{$job->when_price}}',
                    frequency_payments: '{{$job->frequency_payments}}',
                    where_work: '{{$job->where_work}}',
                    over_years_old: '{{$job->over_years_old}}',
                    from_years_old: '{{$job->from_years_old}}',
                    with_violation_health: '{{$job->with_violation_health}}',
        ...
        Vue.createApp(EditJob).mount('#edit_job')
    </script>
