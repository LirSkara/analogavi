...
@endsection
@section('content')
    <div class="container py-lg-5" id="edit_image_estate">
        <div class="d-flex">
        ...
        <div class="d-flex flex-wrap gap-2 mt-3">
            <div v-for="(item, id) in images" :key="item" class="cards shadow-sm bg-white border rounded-3 mt-2">
                <img class="img-fluid rounded-top img-cover w-100" :src="'/storage/realty_image/{{$item->user}}/' + item" alt="...">
                <div class="d-flex">
                    ...
                    <div class="modal-body">
                        ...
                        @csrf
                            <div class="mb-3">
                                ...
                                <div class="text-danger">@{{add_file_error}}</div>
                            </div>
                            <button class="btn btn-primary w-100" v-on:click="add_images(id)">Загрузить</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        const EditImageEstate = {
            ...
            methods: {
                ...
                add_images() {
                    ...
                    if(document.getElementById('add_file').files.length != 0) {
                        if(this.images.length <= 2) {
                            ...
                            axios({
                                method: 'post',
                                url: '/add_image_estate/{{$item->what_i_sell}}/{{$item->sell_and_buy}}/{{$item->id}}',
                                responseType: 'json',
                                data: form_add_image,
                                headers: {
                ...
                delete_images(id) {
                    axios({
                        method: 'post',
                        url: `/delete_image_estate/{{$item->what_i_sell}}/{{$item->sell_and_buy}}/{{$item->id}}/${id}`,
                        responseType: 'json',
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
                ...
                edit_images(id) {
                    ...
                    if(document.getElementById(`edit_file${id}`).files.length != 0) {
                        ...
                        axios({
                            method: 'post',
                            url: `/edit_image_estate/{{$item->what_i_sell}}/{{$item->sell_and_buy}}/{{$item->id}}/${id}`,
                            responseType: 'json',
        ...
        Vue.createApp(EditImageEstate).mount('#edit_image_estate')
    </script>
@endsection